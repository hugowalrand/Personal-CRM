...
const SQL_SCRIPT = `-- Updates the "contacts" table to add the priority feature.
-- This operation will not erase any of your existing data.
ALTER TABLE public.contacts
ADD COLUMN is_priority boolean DEFAULT false;

COMMENT ON COLUMN public.contacts.is_priority IS 'Indicates if the contact is pinned to the top of the list.';
`;

interface UpdateSchemaInstructionsProps {
    onRetry: () => void;
}

export const UpdateSchemaInstructions: React.FC<UpdateSchemaInstructionsProps> = ({ onRetry }) => {
        ...
        <div className="min-h-screen bg-gray-900 flex flex-col justify-center items-center text-gray-200 p-4 font-sans">
            <div className="text-center w-full max-w-3xl bg-gray-800 border border-yellow-500/50 p-4 sm:p-8 rounded-lg shadow-2xl">
                ...
                <p className="text-base sm:text-lg mb-6 text-gray-300">
                    The application has been updated with new features, but your database schema is outdated. The column <code className="bg-gray-700 px-2 py-1 rounded text-yellow-400">is_priority</code> is missing.
                </p>

                <div className="text-left space-y-2 bg-gray-900/50 p-4 rounded-md">
                     <p><strong>To fix this, follow the same steps as for creation:</strong></p>
